class InventoryMailer < ActionMailer::Base
  ...
  def outstanding_request(lender_id)
    ...
    mail(to: "james@projectborrow.com", from: ENV['owner'], :subject => "[Project Borrow]: Accept/ decline requests right in your email!")
    Signup.find_by_id(lender_id).update_attributes(last_emailed_on: Date.today)
